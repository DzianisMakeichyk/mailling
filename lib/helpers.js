...
//`````````````````````````````````````````````````````````````````````Button`````````````````````````````````````````````````````````````````````
Handlebars.registerHelper('makeButton', function(font_size, text_color, align, font_weight, text_transform, width_btn, height_btn, bg_color, border_color, url, options) {
    ...
    var result =''+
    '<!-- Button -->' +
        '<table class="center_button" bgcolor="#' + bg_color + '" width="' + width_btn + '" height="' + height_btn + '" align="' + align + '" border="0" cellpadding="0" cellspacing="0" style="border-radius:25px;border-collapse:separate;border-bottom: 3px solid #' + border_color + '">' +
        '<tr>' +
        '<td mc:edit="text404"  class="text_color_ffffff" align="center" valign="middle" style="color: #ffffff; font-size:' + font_size + 'px; font-weight:' + font_weight + '; font-family: Arial, sans-serif; mso-line-height-rule: exactly;">' +
        '<div class="editable-text">' +
        '<span class="text_container">' +
        '<a href="' + url +'/?utm_content=btn_apl" style="text-decoration: none; color: #' + text_color + '; text-transform: ' + text_transform + '">' +
        '' + options.fn(this) + '' +
        '</a>' +
        '</span>' +
        '</div>' +
...
Handlebars.registerHelper('list', function(list_aligh, dot_color, dot_size, font_color, font_size, font_weight, list_width, context, options) {
    ...
    for(var i=0, j=context.length; i<j; i++) {
        ret = ret + '<li align="' + list_aligh + '" style="font-family: Arial, sans-serif;margin: auto; max-width:' + list_width + ';text-align: ' + list_aligh + ';padding: 0; white-space: normal; color: #' + dot_color + '; line-height: 140%; font-size: ' + dot_size + 'px;list-style-position: inside;">' +
            '<span style="font-family: Arial, sans-serif;margin: auto; max-width:' + list_width + ';color: #' + font_color + '; vertical-align: middle; font-weight: ' + font_weight + '; font-size: ' + font_size + 'px">' + options.fn(context[i]) + '</span></li>';
    }
