...
//`````````````````````````````````````````````````````````````````````Context text new`````````````````````````````````````````````````````````````````````
Handlebars.registerHelper('getOutputByParameters', function(param1, options) {

    return new Handlebars.SafeString(
        '<!-- Context text -->' +
        '<table width="100%" border="0" cellpadding="0" cellspacing="0"  class="full" bgcolor="#ffffff"mc:repeatable="module19">'+
        '<tr>' +
        '<td valign="middle" width="100%" style="text-align:center; font-family:Arial, sans-serif; font-size:20px; ' + param1 + '; line-height: 140%;" class="fullCenter" mc:edit="51">'
        + options.fn(this) +
        '</td>' +
        '</tr>' +
        '</table>' +
...
Handlebars.registerHelper('context_link', function(context_link, font_size, font_weight, text_color, text_align, text_transform, text_decoration, options) {
    ...
    return new Handlebars.SafeString(
        '<!-- Context link -->' +
        '<table width="100%" border="0" cellpadding="0" cellspacing="0" align="' + text_align + '" class="full" bgcolor="#fff
        '<tr>' +
