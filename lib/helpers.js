...
Handlebars.registerHelper('table', function(center, options) {
    ...
    return new Handlebars.SafeString(
        ...
        '<!--[if gte mso 9]> <![endif]-->' +
        '</td>' +
        '<td width="' + center + '%" align="left" style="border-collapse: collapse; mso-table-lspace: 0; mso-table-rspace: 0;">'
        + options.fn(this)
        + '</td>' +
...
//`````````````````````````````````````````````````````````````````````List`````````````````````````````````````````````````````````````````````
Handlebars.registerHelper('list', function(align, options) {
    align = Handlebars.escapeExpression(align);

    return new Handlebars.SafeString(
    '<!-- List -->' +
'<table valign="' + align + '" cellpadding="0" cellspacing="0" border="0" style="margin: 0; padding: 0; width: 100% !important; line-height: 100% !important; border-collapse: collapse; mso-table-lspace: 0pt; mso-table-rspace: 0pt; text-align:' + align + ';">' +
        '<tr valign="' + align + '" style="border-collapse: collapse; mso-table-lspace: 0; mso-table-rspace: 0;text-align:' + align + '">' +
        '<td valign="' + align + '" style="border-collapse: collapse; mso-table-lspace: 0; mso-table-rspace: 0;text-align:' + align + '">'
        + options.fn(this) +
        '</td>' +
        '</tr>' +
        '</table>' +
...
Handlebars.registerHelper('list_post', function(options) {
    ...
    return new Handlebars.SafeString(
        '<!-- List -->' +
        '<span width="15" style=" font-size:' + dot_size + 'px;text-align:' + align + '; vertical-align: top; font-weight: 800; color: #' + dot_color + '; margin: 0; white-space: normal; line-height: 140%; font-family: Arial, sans-serif;">' +
        ' '+ dot +' ' +
        '</span>' +
        '<span style="display:margin: 0; text-align:' + align + '; width:' + width + '; white-space: normal; color: #' + color + '; line-height: 140%; font-weight:' + weight + '; font-family: Arial, sans-serif; font-size:' + font_size + 'px;">' +
         options.fn(this) +
        '</span>' +
    '<!-- End List -->');
});
